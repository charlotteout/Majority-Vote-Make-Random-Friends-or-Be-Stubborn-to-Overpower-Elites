from MM_stab.simulation import simulation_stab
import networkx as nx
import networkit as nk
import os
abs_path = os.path.abspath(os.path.dirname(__file__))

def bigsimulation_stab(graph_loc, type_graph, def_ratio, edge_constant_mult):
    """
    :param graph_loc: string indicating the location of the graph and thus which SN we are considering
    :param type_graph:
    :param def_ratio:
    ...
    final_rounds_list = []
    for init_dens in [0,0.1,0.2,0.3,0.4,0.45,0.472,0.5,0.525,0.55,0.6,0.7,0.8,0.9,1]:
        final_dens = simulation_stab(our_graph=our_graph, initial_prob_dens=init_dens, def_ratio=def_ratio)
        final_rounds_list.append(final_dens)
    print(final_rounds_list)
